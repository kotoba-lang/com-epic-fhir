(ns epic_fhir.main
  "Kotodama WASM entrypoint for the Epic_fhir clean-room actor (L4) — Clojure port.
  ...
  facts (`epic_fhir.<Entity>/<field>`); `*store*` is the in-memory materialization
  ...
  restriction/reason cross-field rule (see `epic_fhir.validation`), all
  ...
  (:require [clojure.string :as str]
            [epic_fhir.validation :as validation]))

(def ns-prefix "epic_fhir")
(def tier "L4")
...
(def entity-specs
  [{:entity "Patient"   :plural "patients"   :id-prefix "epicfhir_pat"
   ...
   ;; HCPCS Level II).
   {:entity "Claim" :plural "claims" :id-prefix "epicfhir_cla"
   ...
   {:entity "Consent" :plural "consents" :id-prefix "epicfhir_cst"
    ...
    :sample {:resourceType "Consent" :patientId "epicfhir_pat_sample0000000000"
   ...
   {:entity "PatientAccessRequest" :plural "patientaccessrequests" :id-prefix "epicfhir_par"
    ...
    :sample {:resourceType "PatientAccessRequest" :patientId "epicfhir_pat_sample0000000000"
...
(defn emit-facts
  "EAVT facts for one record: {\"epic_fhir.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "epic_fhir-compat" :tier tier :entities entities} 200])
...
(defn start! [] :epic_fhir-compat/ready)
